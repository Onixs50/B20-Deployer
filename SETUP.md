# B20 Forge — راهنمای راه‌اندازی (Setup)

> این فایل راهنمای فنی نصب/دیپلوی‌ه. اگه دنبال توضیح کلی «این سایت چیه و چیکار می‌کنه» هستی، برو سراغ [README.md](./README.md).
...
## کانترکت دوم: ارسال گروهی (اختیاری ولی توصیه‌شده)

`contracts/B20BatchDistributor.sol` امکان «ارسال گروهی توی یک تراکنش» رو توی
پنل مدیریت توکن فعال می‌کنه (mint یا transfer به چند آدرس با یک امضا، نه
یکی‌یکی). دیپلویش دقیقاً مثل بالا:

1. فایل رو توی Remix باز کن، Compile با `0.8.24`.
2. روی هر دو شبکه (Sepolia و Mainnet) جدا دیپلوی کن.
3. آدرس هرکدوم رو بذار توی `.env`:
   `VITE_BATCH_DISTRIBUTOR_ADDRESS_TESTNET` و
   `VITE_BATCH_DISTRIBUTOR_ADDRESS_MAINNET`.

اگه این دو مقدار خالی بمونن، بقیهٔ سایت عادی کار می‌کنه؛ فقط تب «ارسال گروهی»
توی پنل مدیریت غیرفعال می‌مونه.
...
4. **`VITE_CREATOR_TWITTER` / `VITE_CREATOR_GITHUB`** — لینک‌های خودت، پایین
   سایت کنار «ساخته‌شده توسط Onixia» نشون داده می‌شن. اختیاری‌ان.
...
- **دو زبانه:** دکمه‌ی «فا / EN» بالای سایت زبان و جهت متن (RTL/فارسی، LTR/انگلیسی)
  رو عوض می‌کنه؛ انتخاب کاربر توی مرورگرش ذخیره می‌مونه.
- **تاریخچه‌ی توکن‌ها:** دکمه‌ی شناور پایین سمت چپ، توکن‌هایی که همون کیف‌پول از
  همون مرورگر دیپلوی کرده رو نشون می‌ده (ذخیره در `localStorage`، نه از روی
  زنجیره — یعنی مخصوص همون مرورگره، بین دستگاه‌ها sync نمی‌شه).
- **دیپلوی بدون رفرش:** بعد از هر دیپلوی موفق، دکمه‌ی «دیپلوی یه توکن دیگه»
  فرم رو کامل ریست می‌کنه تا بدون رفرش صفحه بشه دوباره یه توکن جدید ساخت.
